...
fn build(config: Config) -> [u16;SPEED_TABLE_LEN] {
    ...
    table
        ...
        .take(MIDPOINT)
        .for_each(|(i, v)|
            *v = calc_step(v_min, v_mid, i - 1) // -1 because we skip the first entry
        );
    ...
    table
        ...
        .skip(MIDPOINT + 1)
        .for_each(|(i, v)|
            *v = calc_step(v_mid, v_max, i - MIDPOINT) // skip the first half
        );
...
mod tests {
    ...
    fn table_matches_index_in_simple_first_segment_case() {
        ...
        let cfg = Config { v_start: 1, v_mid: 64, v_high: 126 };
        let table = build(cfg);

        // index 0 is always 0
        assert_eq!(
            table.as_slice(),
            &[0, 1, 17, 33, 49, 65, 82, 98, 114, 130, 147, 163, 179, 195, 212, 228, 244, 260, 277, 293, 309, 325, 341, 358, 374, 390, 406, 423, 439, 455, 471, 488, 504, 520, 536, 553, 569, 585, 601, 618, 634, 650, 666, 682, 699, 715, 731, 747, 764, 780, 796, 812, 829, 845, 861, 877, 894, 910, 926, 942, 959, 975, 991, 1007, 1039, 1055, 1071, 1086, 1102, 1118, 1134, 1149, 1165, 1181, 1197, 1212, 1228, 1244, 1260, 1275, 1291, 1307, 1323, 1338, 1354, 1370, 1386, 1401, 1417, 1433, 1449, 1464, 1480, 1496, 1512, 1527, 1543, 1559, 1575, 1590, 1606, 1622, 1638, 1653, 1669, 1685, 1701, 1716, 1732, 1748, 1764, 1779, 1795, 1811, 1827, 1842, 1858, 1874, 1890, 1905, 1921, 1937, 1953, 1968, 1984, 2000, 2016]
        );
    ...
    fn two_segment_table_with_different_slopes() {
        // Slopes: (50-10) != (200-50)
        let cfg = Config { v_start: 10, v_mid: 50, v_high: 200 };
